...
add_action('admin_head', array('Newfashion_MetaBox', '_global_head'));

add_action('admin_footer', array('Newfashion_MetaBox', '_global_foot'));
...
class Newfashion_MetaBox
{
	...
	function Newfashion_MetaBox($arr)
	{
	...
	function _init()
	{
		...
		if ( ! Newfashion_MetaBox::_is_post() AND ! Newfashion_MetaBox::_is_page()) return;
	...
	{
		if ('post' == Newfashion_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == Newfashion_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = Newfashion_MetaBox::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = Newfashion_MetaBox::_get_post_id();
	...
	function _save($post_id) 
	{
		...
		Newfashion_MetaBox::clean($new_data);
	...
	function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i])) 
				{
					Newfashion_MetaBox::clean($arr[$i]);
		...
		?>
				<li class="tabs-<?php echo esc_attr($element['id']); ?><?php echo ($count==1)? ' active': ''?>">
					<a href="#<?php echo esc_attr($element['id']); ?>">
						...
						<?php echo preg_replace("#\s+#", ' ', $element['title']); ?>
					</a>
				</li>
		<?php
			$count++;       
	...
	?>
        ...
        <select name="<?php $this->the_name(); ?>" id="<?php echo esc_attr($item['id']); ?>">
	        <?php foreach($item['data'] as $layout){ ?>
	            <option value="<?php echo esc_attr($layout['id']); ?>" <?php $this->the_select_state($layout['id']); ?>><?php echo trim($layout['name']); ?></option>
	        <?php } ?>
        </select>
	...
	?>
        ...
        <label for="meta-checkbox">
            <input type="checkbox" name="<?php $this->the_name(); ?>" id="<?php echo esc_attr($item['id']); ?>" value="1" <?php $this->the_checkbox_state($item['id'], 1); ?>>
	...
	?>
		<label><?php echo ($item['title']); ?></label>
        <input type="text" name="<?php $this->the_name(); ?>" style="width: 500px" id="<?php echo esc_attr($item['id']);?>" value="<?php echo esc_attr($this->the_value()); ?>"/>
        <br/><?php echo preg_replace("#\s+#", ' ', $item['des']); ?>
    <?php
	}
	...
	?>
		<label><?php echo ($item['title']); ?></label>
        <input type="number" name="<?php $this->the_name(); ?>" style="width: 100px" id="<?php echo esc_attr($item['id']);?>" value="<?php echo esc_attr($this->the_value()); ?>"/>
        <br/><?php echo preg_replace("#\s+#", ' ', $item['des']); ?>
        <p>
    <?php
	}
	...
	?>
		<label><?php echo ($item['title']); ?></label>
        <textarea class="textarea " style="width: 500px; height: 100px" name="<?php $this->the_name(); ?>" id="<?php echo esc_attr($item['id']);?>"><?php echo esc_attr($this->the_value()); ?></textarea>
	<?php
	}
	...
	function addColorElement( $item) {
		$this->the_field( $item['id']);
		$this->wp_enqueue_color_picker();
		add_action( 'admin_enqueue_scripts', 'wp_enqueue_color_picker' );

		?>
		<label><?php echo esc_html( $item['title'] );?></label>
        <input type="text" value="<?php echo esc_attr($this->the_value()); ?>" class="wp-color-picker-field" data-default-color="#ffffff" name="<?php $this->the_name(); ?>"/>
        <?php
	}
	...
	//Add date field
	function wpo_datepicker( $data ) {
		wp_enqueue_script( 'jquery-ui-datepicker' );
		$this->the_field( $data['id']);
	?>
		<label><?php echo ($data['title']); ?></label>
			<span class="wpo_datetimepicker_block">          
				<input id="wpo_datetime" name="<?php $this->the_name(); ?>" type="text" value="<?php echo esc_attr($this->the_value()); ?>"/>
            </span>
    <?php
	}
...
?>
		<p class="wpo_section page-layout">
		    <?php $this->the_field( $item['id']); ?>
	        <label><?php echo ( $item['title']); ?></label>
		    <br>
		    <br>
			<img  src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'1col.png'; ?>" class="layout" data-value="fullwidth" alt="">
			<img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'2cl.png'; ?>" class="layout" data-value="leftmain" alt="">
			<img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'2cr.png'; ?>" class="layout" data-value="mainright" alt="">
			<img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'3c.png'; ?>" class="layout" data-value="leftmainright" alt="">
		    <select  name="<?php $this->the_name(); ?>" style="display:none;">
		    	<option value="fullwidth" <?php $this->the_select_state('fullwidth'); ?>><?php echo __('Fullwidth','newfashion'); ?></option>
		    	<option value="leftmain" <?php $this->the_select_state('leftmain'); ?>><?php echo __('Left Sidebar','newfashion'); ?></option>
		    	<option value="mainright" <?php $this->the_select_state('mainright'); ?>><?php echo __('Right Sidebar','newfashion'); ?></option>
		    	<option value="leftmainright" <?php $this->the_select_state('leftmainright'); ?>><?php echo __('Left & Right Sidebar','newfashion'); ?></option>
		    </select>
		</p>
<?php
	...
	function wpo_image_upload( $data ){
		$this->the_field( $data['id']);
		$this->wpo_image_enqueue();
?>
		<label><?php echo esc_html( $data['title'] );?></label>
	    <input type="hidden" name="<?php $this->the_name(); ?>" id="meta-image" value="<?php echo esc_attr($this->the_value()); ?>" />
	    <a class="button" id="meta-image-button"><i class='fa fa-upload'></i><?php echo ( $data['description'] ); ?></a>
     	<?php if($data['sub']): ?>
    	<br/><?php echo preg_replace("#\s+#", ' ', $data['sub']); ?>
    	<?php endif; ?>
     	<?php if( $this->get_the_value()): ?><br/>
     		<span class="image"><img src="<?php echo esc_url_raw($this->the_value()); ?>"></span>
 		<?php endif; ?>
<?php
	}

	public function wpo_image_enqueue() {
 
        // Registers and enqueues the required javascript.
        wp_register_script( 'newfashion-meta-box-image', WPO_FRAMEWORK_ADMIN_STYLE_URI. 'js/meta-box-image.js', array( 'jquery' ) );
        wp_localize_script( 'newfashion-meta-box-image', 'meta_image',
            array(
                'title' => __( 'Choose or Upload an Image', 'newfashion' ),
                'button' => __( 'Use this image', 'newfashion'),
            )
        );
        wp_enqueue_script( 'newfashion-meta-box-image' );
        add_action( 'admin_enqueue_scripts', 'wpo_image_enqueue' );
    }
