...
if(class_exists('PostRatings')){
    ...
    class Newfashion_Top_Rate_Widget extends Newfashion_Widget {
        public function __construct() {
            parent::__construct(
                // Base ID of your widget
                'newfashion_top_rate_widget',
                // Widget name will appear in UI
                __('WPO Top Rate Widget', 'newfashion'),
                // Widget description
                array( 'description' => __( 'The highest rated posts on your site', 'newfashion' ), )
            );
        ...
        public function widget( $args, $instance ) {
            ...
            $title      = apply_filters('widget_title', empty($instance['title']) ? __('Top Rated', 'newfashion') : $instance['title'], $instance, $this->id_base);
        ...
        public function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php _e('Get most rated:', 'newfashion'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('sort')); ?>"><?php _e('Sort by:', 'newfashion'); ?></label>
                <select class="widefat" id="<?php echo esc_attr($this->get_field_id('sort')); ?>" name="<?php echo esc_attr($this->get_field_name('sort')); ?>">
                <option <?php selected('bayesian_rating', $instance['sort']); ?> value="bayesian_rating"><?php _e('Overall bayesian rating (score)', 'newfashion'); ?></option>
                <option <?php selected('rating', $instance['sort']); ?> value="rating"><?php _e('Average rating', 'newfashion'); ?></option>
                <option <?php selected('votes', $instance['sort']); ?> value="votes"><?php _e('Number of votes', 'newfashion'); ?></option>
                </select>
            </p>
            ...
            <p>
                ...
                <?php _e('Ascending', 'newfashion'); ?>
                </label>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('date_limit')); ?>"><?php _e('Ignore posts older than:', 'newfashion'); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id('date_limit')); ?>" name="<?php echo esc_attr($this->get_field_name('date_limit')); ?>" type="text" value="<?php echo esc_attr( $instance['date_limit'] ); ?>" size="3" /> <?php _e('days', 'newfashion'); ?>
                <br />
                <small><?php _e('(0 to ignore date)', 'newfashion'); ?></small>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('Limit:', 'newfashion'); ?></label>
    ...
    register_widget( 'Newfashion_Top_Rate_Widget' );
}