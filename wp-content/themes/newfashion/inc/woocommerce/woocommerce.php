...
if(class_exists('WooCommerce')){
	...
	class Newfashion_Woocommerce{
		public static function getInstance(){
			static $_instance;
			if( !$_instance ){
				$_instance = new Newfashion_Woocommerce();
			}
			return $_instance;
		}

		public function __construct(){
			if(newfashion_wpo_theme_options('is-quickview',true)){
				// Ajax Quickview
			...
			add_action( 'wpo_button_display',array('Newfashion_Woocommerce','renderButton'),20 );


			add_action( 'widgets_init', array($this,'override_woocommerce_widgets'), 15 );

			if( newfashion_wpo_theme_options( 'is-swap-effect',true ) ){
		...
		public function initScripts(){
			wp_enqueue_script('newfashion-quickview_js', NEWFASHION_WPO_THEME_URI.'/js/woocommerce.js',array(),false,true);
		}

		public function wc_product_per_page($cols){
			return newfashion_wpo_theme_options('woo-number-page',12);
		}

		public function wc_loop_shop_columns(){
			return newfashion_wpo_theme_options('wc_itemsrow',3);
		}
		...
		public function QuickView(){
			...
			if($query->have_posts()){
				while($query->have_posts()): $query->the_post(); global $product;
					if(is_file(NEWFASHION_WPO_THEME_DIR.'/woocommerce/quickview.php')){
						require(NEWFASHION_WPO_THEME_DIR.'/woocommerce/quickview.php');
					}
				endwhile;
				wp_reset_postdata();
			}
	...
	Newfashion_Woocommerce::getInstance();
}