...
$tabs_nav = '';
$output .= '<div class="tab-v8">';
$tabs_nav .= '<ul class="nav nav-tabs wpb_tabs_nav ui-tabs-nav vc_clearfix">';
foreach ( $tab_titles as $tab ) {
	$tab_atts = shortcode_parse_atts($tab[0]);
   if(isset($tab_atts['icon']) && ($tab_atts['icon'])) {
      $icon = '<i class ="space-padding-lr-5 fa '.$tab_atts['icon'].'"></i>';
   }else $icon = '';
