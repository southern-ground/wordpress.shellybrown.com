...
<?php while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>
	...
	<div class="<?php echo esc_attr( $class_column ).esc_attr( $class_fix ); ?> wow fadeInUp product-wrapper" data-wow-duration="0.6s" data-wow-delay="<?php echo esc_attr( $_delay ); ?>ms">
		<?php wc_get_template_part( 'content', 'product-grid' ); ?>
	</div>
	<?php if( (($_count%$columns_count==0 && $_count!=1) || $_count== $posts_per_page || $_count==$_total ) && $columns_count != 1) echo '</div>'; ?>
